...
우선 `key_by` 함수를 통해 Table의 `key`를 지정해야 합니다. 다음은 `gene_expr` Table에 대해서 key를 지정하는 코드입니다. `describe` 함수를 통해서 `key`가 잘 설정이 되었는지 확인할 수 있습니다. `key_by`에 대한 인수로 `'id'`처럼 field name을 바로 입력해도 되고, 또는 `gene_expr.id`처럼 attribute 형태로 입력해도 됩니다.
