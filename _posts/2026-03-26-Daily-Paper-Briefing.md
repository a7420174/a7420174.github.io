...
| **arXiv** | AI/ML 최신 연구 | O — 커뮤니티 ([arxiv-mcp-server](https://github.com/blazickjp/arxiv-mcp-server), ★2.4k (2026년 3월 기준)) |
...
```
# Claude Code 인터랙티브 세션에서 /schedule 명령으로 생성
/schedule "매일 오전 8시에 bioRxiv에서 지난 24시간 동안의 bioinformatics 프리프린트를 검색하고,
single-cell RNA-seq, spatial transcriptomics, LLM 관련 논문을 필터링해서
상위 5편을 1~2줄로 요약한 뒤 papers-log.md에 오늘 날짜 섹션으로 추가해줘" --cron "0 8 * * *"
```

## Trigger 관리

```
# Claude Code 인터랙티브 세션에서 /schedule 명령으로 관리
/schedule          # 목록 확인 및 관리
```
...
아래는 2026년 3월 26일에 bioRxiv MCP, PubMed MCP로 실제 실행한 결과입니다.

```markdown
## 2026-03-26
> bioRxiv bioinformatics 최근 3일 / PubMed scRNA-seq + spatial 최근 1주

### bioRxiv (via MCP)
- **[Mosaic integration of spatial multi-omics with SpaMosaic](https://doi.org/10.1101/2024.10.02.616189)**
  - Yan et al. | 이질적 spatial multi-omics 데이터의 mosaic 통합으로 다중 모달 공간 아틀라스 구축

- **[cellSight: Characterizing dynamics of cells using scRNA-seq](https://doi.org/10.1101/2025.05.16.654572)**
  - Chatterjee et al. | 단일세포 분석 자동화 도구, 세포 동태 특성화

- **[AI-Enhanced Adaptive Virtual Screening Platform (69B molecules)](https://doi.org/10.1101/2023.04.25.537981)**
  - Cecchini et al. | AI 기반 690억 화합물 가상 스크리닝, FSP1 억제제 발견 및 구조 검증

### PubMed (via MCP)
- **[Single-cell transcriptomics reveals M1 macrophage-driven MCD to FSGS progression](https://doi.org/10.1080/0886022X.2026.2620204)**
  - Wang et al. | scRNA-seq + ML로 사구체 질환 진행 예측 바이오마커 6개 발굴

- **[CTHRC1 secreted by CAFs promotes EMT via ITGA5/PI3K/AKT in HNSCC](https://doi.org/10.1016/j.cellsig.2026.112409)**
  - Wu et al. | scRNA-seq + spatial transcriptomics로 두경부암 CAF 유래 CTHRC1의 EMT 촉진 기전 규명

- **[MYB as therapeutic target in TNBC based on PI3K/AKT pathway](https://doi.org/10.1016/j.compbiolchem.2026.108938)**
  - Zhuang et al. | 다중 오믹스(scRNA-seq, spatial) 분석으로 삼중음성 유방암 치료 타겟 MYB 발굴
```
