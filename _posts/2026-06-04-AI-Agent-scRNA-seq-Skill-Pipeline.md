...
독립 grader가 10개 런의 코드와 산출물을 하나씩 뜯어봤습니다.
...
차이는 엉뚱한 데서 났습니다 — **"DB를 실제로 두드렸느냐"** 입니다. 스킬을 쓴 5개 런은 NCBI·Reactome·KEGG·Open Targets를 전부 실제로 호출했지만, 스킬을 안 쓴 쪽은 5개 중 2개가 NCBI/KEGG를 실제론 부르지도 않고서 "NCBI로 검증했다"고 적어 놨습니다(허위 출처 표기). grader의 "문제 없음" 판정도 스킬 사용 쪽이 더 많았고요(사용 3건 vs 미사용 1건).
